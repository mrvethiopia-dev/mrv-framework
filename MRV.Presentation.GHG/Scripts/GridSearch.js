...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	...
	,onTriggerSearch:function() {
		...
		// ask server to filter records
			...
			delete (store.baseParams[this.paramNames.record]);
			if (store.lastOptions && store.lastOptions.params) {
				delete (store.lastOptions.params[this.paramNames.record]);
			}
			if(fields.length) {
				store.baseParams[this.paramNames.record] = Ext.encode({ fields: Ext.encode(fields), value: val });
			}
