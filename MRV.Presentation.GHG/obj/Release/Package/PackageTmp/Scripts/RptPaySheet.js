...
Ext.ns('Ext.core.finance.ux.rptSheetFilterCriteria');
...
Ext.core.finance.ux.rptPaySheet.Form = function (config) {
    Ext.core.finance.ux.rptPaySheet.Form.superclass.constructor.call(this, Ext.apply({
        ...
        baseCls: 'x-plain',
        items: [{
            id: 'rptSheetPeriodId', xtype: 'combo', anchor: '100%', fieldLabel: 'Select Period', triggerAction: 'all', mode: 'local', editable: false, typeAhead: true,
...
* @namespace Ext.core.finance.ux.rptPaySheet
* @class     Ext.core.finance.ux.rptPaySheet.Panel
* @extends   Ext.Panel
*/
...
Ext.extend(Ext.core.finance.ux.rptPaySheet.Panel, Ext.Panel, {
    initComponent: function () {
        ...
        this.FilterGrid = new Ext.core.finance.ux.rptSheetFilterCriteria.Grid();
...
Ext.core.finance.ux.rptSheetFilterCriteria.Store = function (config) {
    Ext.core.finance.ux.rptSheetFilterCriteria.Store.superclass.constructor.call(this, Ext.apply({
...
Ext.extend(Ext.core.finance.ux.rptSheetFilterCriteria.Store, Ext.data.DirectStore);
Ext.reg('rptSheetFilterCriteria-store', Ext.core.finance.ux.rptSheetFilterCriteria.Store);

/**
* @desc      rptSheetFilterCriteria grid
* @author    Dawit Kiros
* @copyright (c) 2014, LIFT
* @date      November 01, 2013
* @namespace Ext.core.finance.ux.rptSheetFilterCriteria
* @class     Ext.core.finance.ux.rptSheetFilterCriteria.Grid
* @extends   Ext.grid.GridPanel
*/
Ext.core.finance.ux.rptSheetFilterCriteria.Grid = function (config) {
    Ext.core.finance.ux.rptSheetFilterCriteria.Grid.superclass.constructor.call(this, Ext.apply({
        store: new Ext.data.DirectStore({
            directFn: Tsa.GetRptDepartments,
            paramsAsHash: false,
            paramOrder: 'start|limit|sort|dir|record',
            ...
            listeners: {
                beforeLoad: function () {
                    Ext.getCmp('rptSheetFilterCriteria-grid').body.mask('Loading...', 'x-mask-loading');
                },
                load: function () {
                    var grid = Ext.getCmp('rptSheetFilterCriteria-grid');
                    grid.body.unmask();
                    Ext.core.finance.ux.SystemMessageManager.hide();
                },
                loadException: function () {
                    Ext.getCmp('rptSheetFilterCriteria-grid').body.unmask();
                },
                scope: this
            }
        }),
        id: 'rptSheetFilterCriteria-grid',
        pageSize: 10,
        //height: 700,  Removing this property will set the hight of the grid to be equal with the size of its parent panel
        stripeRows: true,
        columnLines: true,
        border: true,
        clicksToEdit: 1,
...
Ext.extend(Ext.core.finance.ux.rptSheetFilterCriteria.Grid, Ext.grid.EditorGridPanel, {
    initComponent: function () {
        ...
        this.tbar = ['Filter By', {
            ...
            listeners: {
                select: function (combo, record, index) {
                    ...
                    var selText = Ext.getCmp('cmbSheetFilterBy').lastSelectionText;
                    ...
                    if (selText == 'Departments') {
                        Tsa.GetRptDepartments(function (response) {
                            ...
                            var grid = Ext.getCmp('rptSheetFilterCriteria-grid');
                            var store = new Ext.core.finance.ux.rptSheetFilterCriteria.Store({
                                fields: (function () {
                                    var fields = [];
            ...
            disabled: !Ext.core.finance.ux.Reception.getPermission('Pay Sheets', 'CanView')
        }, {
            xtype: 'tbseparator'
        }, {
            id: 'cmdSheetReset',
            ...
            handler: function () {
                ...
                Ext.getCmp('rptSheetFilterCriteria-grid').getStore().removeAll();
                //                Ext.getCmp('rptSheetFilterCriteria-paging').doRefresh();
            },
            scope: this
        }];

        this.bbar = new Ext.PagingToolbar({
            id: 'rptSheetFilterCriteria-paging',
            store: this.store,
            displayInfo: false,
        ...
        Ext.core.finance.ux.rptSheetFilterCriteria.Grid.superclass.initComponent.apply(this, arguments);
    },
    onPreviewClick: function () {
        ...
        var periodId = Ext.getCmp('rptSheetPeriodId').getValue();
        var batchId = Ext.getCmp('rptBatchId').getValue();
        var periodName = Ext.getCmp('rptSheetPeriodId').getRawValue();
        var filterBy = Ext.getCmp('cmbSheetFilterBy').getRawValue();
        var grid = Ext.getCmp('rptSheetFilterCriteria-grid');
        ...
        var selectedReport = 'rpt_PaySheet';
        PayrollReports.ViewReport(selectedReport, periodId, periodName, filterBy, GroupId,0,batchId, function (result, response) {
            if (result.success) {
                var url = result.URL;
...
Ext.reg('rptSheetFilterCriteria-grid', Ext.core.finance.ux.rptSheetFilterCriteria.Grid);

Ext.core.finance.ux.rptSheetFilterCriteria.Panel = function (config) {
    Ext.core.finance.ux.rptSheetFilterCriteria.Panel.superclass.constructor.call(this, Ext.apply({
